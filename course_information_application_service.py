...
def check_availability_of_courses(session: requests.Session, args: tuple[str]) -> None:
    ...
    for course in args:
        ...
        url = (f"https://e-learning.tuhh.de/studip/dispatch.php/globalsearch/find/100?search={course}&filters=%7B"
               f"%22category%22%3A%22show_all_categories%22%2C%22semester%22%3A%22%22%7D")
        ...
        for item in content:
            if "23/24" in item["date"]:
                ...
                pass
            url_course = f"https://e-learning.tuhh.de/studip/dispatch.php/course/details/index/{item['id']}"
...
def attempt_apply(session: requests.Session, item: dict, zugang: dict) -> None:
    url_apply = f"https://e-learning.tuhh.de/studip/dispatch.php/course/enrolment/apply/{item['id']}"
